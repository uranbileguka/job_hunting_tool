export const migrations = [
  ...
  {
    id: "007_add_export_directory_to_job_application",
    sql: `
      ALTER TABLE job_application
      ADD COLUMN IF NOT EXISTS export_directory TEXT NOT NULL DEFAULT '';
    `
  },
  {
    id: "008_create_app_settings_table",
    sql: `
  ...
  {
    id: "009_create_user_urls_table",
    sql: `
      CREATE TABLE IF NOT EXISTS user_urls (
        user_id INTEGER PRIMARY KEY DEFAULT 1,
        template_root TEXT NOT NULL DEFAULT '',
        export_root TEXT NOT NULL DEFAULT '',
        updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
      );

      INSERT INTO user_urls (user_id, template_root, export_root)
      VALUES (
        1,
        COALESCE((SELECT value FROM app_settings WHERE key = 'template_root'), ''),
        COALESCE((SELECT value FROM app_settings WHERE key = 'export_root'), '')
      )
      ON CONFLICT (user_id)
      DO NOTHING;
    `
  ...
  {
    id: "016_add_resume_default_root_to_user_urls",
    sql: `
      ALTER TABLE user_urls
      ADD COLUMN IF NOT EXISTS resume_default_root TEXT NOT NULL DEFAULT '';
    `
  },
  {
    id: "017_add_resume_pdf_root_to_user_urls",
    sql: `
      ALTER TABLE user_urls
      ADD COLUMN IF NOT EXISTS resume_pdf_root TEXT NOT NULL DEFAULT '';

      UPDATE user_urls
      SET resume_pdf_root = COALESCE(NULLIF(resume_pdf_root, ''), resume_default_root)
      WHERE COALESCE(resume_default_root, '') <> '';
    `
  },
  {
    id: "018_add_export_paths_to_job_application",
    sql: `
      ALTER TABLE job_application
      ADD COLUMN IF NOT EXISTS cover_letter_docx_path TEXT NOT NULL DEFAULT '';

      ALTER TABLE job_application
      ADD COLUMN IF NOT EXISTS cover_letter_pdf_path TEXT NOT NULL DEFAULT '';

      ALTER TABLE job_application
      ADD COLUMN IF NOT EXISTS resume_docx_path TEXT NOT NULL DEFAULT '';

      ALTER TABLE job_application
      ADD COLUMN IF NOT EXISTS resume_pdf_path TEXT NOT NULL DEFAULT '';
    `
  },
  {
    id: "019_add_resume_summary_to_job_application",
    sql: `
      ALTER TABLE job_application
  ...
  {
    id: "020_create_application_state_log",
    sql: `
      ...
      CREATE INDEX IF NOT EXISTS idx_application_state_log_application_id
        ON application_state_log(application_id, changed_at DESC);
    `
  }
];
